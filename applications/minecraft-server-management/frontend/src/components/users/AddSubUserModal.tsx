...
import { SUBUSER_PERMISSIONS_SCHEMA } from '../../types';

interface AddSubUserModalProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (data: { username: string; email?: string; permissions: string[] }) => Promise<void>;
  loading: boolean;
}

export const AddSubUserModal: React.FC<AddSubUserModalProps> = ({
  open,
  onClose,
  onSubmit,
  loading,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!username.trim()) {
      setError('Please enter a username for the collaborator.');
      return;
    }
    setError(null);
    await onSubmit({
      ...
      <form onSubmit={handleSubmit}>
        <DialogTitle sx={{ pb: 1 }}>
          ...
          <Typography variant="caption" sx={{ color: '#94a3b8' }}>
            Grant granular server control, file management, and backup permissions to collaborators.
          </Typography>
        </DialogTitle>
          ...
          <Stack spacing={3}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2}>
              <TextField
                ...
                placeholder="e.g. steve, moderator_alex"
                value={username}
                      ...
                      {group.permissions.map((p) => {
                        ...
                        return (
                          <Box
                            ...
                            sx={{
                              p: 1.5,
                              backgroundColor: isChecked ? 'rgba(16, 185, 129, 0.08)' : 'rgba(0, 0, 0, 0.2)',
                          ...
                          >
                            <Checkbox
                              checked={isChecked}
                              size="small"
                              sx={{ p: 0, mt: 0.2, color: '#64748b', '&.Mui-checked': { color: '#10b981' } }}
                            />
                            <Box>
        ...
        <DialogActions sx={{ p: 2.5, borderColor: 'rgba(255, 255, 255, 0.08)' }}>
          ...
          >
            {loading ? 'Inviting Sub-User...' : 'Add Sub-User'}
          </Button>
        </DialogActions>
      </form>
    </Dialog>
